...
const { getFoundingCircleContract } = require("../blockchain.service");
...
const syncFoundingCircleOwnersState = async () => {
    ...
    for (let tokenId = 1; tokenId <= totalSupply; tokenId += 1) {
        let owner = "";
        try {
            owner = normalizeWallet(await contract.ownerOf(tokenId));
        } catch (_) {
            continue;
        }
        ...
        const [timeRemainingRaw, pendingRewardsRaw, metadataUrl] = await Promise.all([
            contract.getTimeRemaining(tokenId).catch(() => 0n),
            contract.getPendingReward(tokenId).catch(() => 0n),
            resolveMetadataUrlForToken(contract, tokenId).catch(() => "")
        ]);
    ...
    if (tokens.length > 0) {
        await FoundingCircle.bulkWrite(
            tokens.map((item) => ({
                updateOne: {
    ...
    if (owners.length > 0) {
        const regaliasResult = await FoundingCircle.aggregate([
